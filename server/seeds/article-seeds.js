...
export const seedArticles = async (users) => {
  ...
  const category = await Category.create({ categoryName: 'Tech' });

  const tag1 = await Tag.create({ tagText: 'andela' });
  const tag2 = await Tag.create({ tagText: 'bootcamp' });
  const tag3 = await Tag.create({ tagText: 'epic values' });
  ...
  const createdArticles = dummyArticles.map(async (articleEntry) => {
    ...
    await newArticle.addTags([tag1, tag2, tag3]);

    // last version of the comment
